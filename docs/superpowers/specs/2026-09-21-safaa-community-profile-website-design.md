...
The approved Gemini Trust composition and English/Chinese, multi-city content goals remain unchanged. During source verification, unrelated imagery was removed from the published pages. The current implementation uses the approved SAFAA workshop photograph for the hero and mission section; every selected activity now uses an exact local event image, preferring a live event photograph and falling back only to that activity's matching poster. No city-cover or cross-event image is used.

The curated collection now contains 18 source-checked activities spanning 2024–2026, five cities and online; three featured activities cover Sydney, Melbourne and Brisbane. The collection intentionally foregrounds workplace, AI and practical-skills programming while retaining community and wellbeing breadth. The production routes are `/events/` and `/zh/events/`, retaining the approved Gemini reference's “Events” route naming. The compact launch content model is a typed `src/data/site.ts` array with a `featured` flag, bilingual detail and source URL, rather than one JSON file per event. These choices supersede the data-file and route examples below; the maintenance workflow is recorded in `docs/content-update.md`.
